...
class DamageInference:
    def __init__(self, model_path, device='auto'):
        ...
        # Load model
        self.model = SingleHeadResNet(num_classes=3)
        self.model.load_state_dict(torch.load(model_path, map_location=self.device))
        self.model.to(self.device)
        self.model.eval()
        
        # Class mappings
        self.idx_to_class = {0: '18', 1: '19', 2: '20'}
    ...
    def predict_single_image(self, image_path):
        """Predict damage class for single image"""
        try:
            ...
            with torch.no_grad():
                ...
                return {
                    ...
                    'damage_name': self.class_names[pred_class],
    ...
    def create_simple_csv(self, results, output_file):
        """Create simple CSV: ID, Damage_Class"""
        
        with open(output_file, 'w') as f:
            f.write('ID,Damage_Class\n')
            
            for i, result in enumerate(results, 1):
                if result['success']:
                    f.write(f'{i},{result["damage_class"]}\n')
                else:
                    f.write(f'{i},-1\n')  # Error indicator
        ...
        for i, result in enumerate(results, 1):
            if result['success']:
                csv_data.append({
                    ...
                    'Damage_Name': result['damage_name'],
    ...
    def print_summary(self, results):
        ...
        print(f"\n{'='*60}")
        print(f"DAMAGE CLASSIFICATION RESULTS")
        print(f"{'='*60}")
        ...
        if successful:
            ...
            for damage_class in sorted(class_counts.keys()):
                count = class_counts[damage_class]
                percentage = count / len(successful) * 100
                damage_name = self.class_names.get(damage_class, damage_class)
                print(f"  Class {damage_class} ({damage_name}): {count} ({percentage:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(description='Damage Classification Inference')
    parser.add_argument('--model', required=True, help='Path to trained model (.pth file)')
    parser.add_argument('--input', required=True, help='Input image or directory')
    parser.add_argument('--output', default='damage_results.csv', help='Output CSV file')
    parser.add_argument('--simple', action='store_true', help='Create simple CSV (ID, Damage only)')
    parser.add_argument('--detailed', action='store_true', help='Create detailed CSV with confidence scores')
    
    args = parser.parse_args()
    
    # Initialize inference
    inference = DamageInference(args.model)
    ...
    if os.path.isfile(args.input):
        ...
        if result['success']:
            ...
            print(f"  Damage: Class {result['damage_class']} - {result['damage_name']} (confidence: {result['confidence']:.3f})")
        else:
        ...
        if successful_results:
            ...
            for i, result in enumerate(successful_results[:5], 1):
                print(f"  {i}. {result['image_name']}: Damage={result['damage_class']}")
...
#python damage_inference.py --model ./best_damage_model.pth --input /root/ak/column --output ./damage_results.csv --detailed